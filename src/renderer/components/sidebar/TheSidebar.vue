<template>
    <div ref="sidebarRef" class="sidebar">
        <SidebarList v-model="activeTab" :tabs="tabs" :is-system="true">
        ...
        >
            ...
            >
                <template #default="{ node }">
                    ...
                    >
                        <TheFolder :id="node.id" :name="node.name" />
                    </SidebarListItem>
                </template>
            </AppTree>
        </SidebarList>
        <div ref="gutterRef" class="gutter-line" />
    </div>
</template>
...
const onDrop = async (e: DragEvent, id: string) => {
    ...
    if (payload) {
        ...
        for (const i of snippetIds) {
            const isDeleted = snippetStore.snippets.find(
                (s) => s.id === i
            )?.isDeleted
